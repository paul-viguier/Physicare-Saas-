...
import { AppHeader, SpaceNav, Loader, styles, COLORS, ADMIN_NAV, STAGE_COLOR } from '@/core/ui'
        ...
        {orgs && (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(320px, 1fr))', gap:16 }}>
            {orgs.map(o => {
              ...
              return (
                <Link key={o.id} href={`/admin/org/${o.id}`} style={{ textDecoration:'none' }}>
                  <div style={{ ...styles.card, borderLeft:`4px solid ${color}`, cursor:'pointer', transition:'transform .15s' }}
                    ...
                    <div style={{ fontSize:12, color:COLORS.purple, fontWeight:800, marginTop:12 }}>Voir le détail →</div>
                  </div>
                </Link>
              )
            })}
